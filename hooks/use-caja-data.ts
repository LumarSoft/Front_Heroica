...
function getEndpoints(tipo: 'efectivo' | 'banco') {
  if (tipo === 'banco') {
    return {
      ...
      getTotales: (sucursalId: number, moneda: string) =>
...
export function useCajaData(
  ...
  const [isNuevoMovimientoDialogOpen, setIsNuevoMovimientoDialogOpen] =
    useState(false);
  const [isMoverMovimientoDialogOpen, setIsMoverMovimientoDialogOpen] =
    useState(false);
  ...
  const { saldoNecesarioFiltrado, saldoNecesarioSinDeudaFiltrado } =
    useMemo(() => {
