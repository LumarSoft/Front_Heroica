        ...
        {step === 'setup' && (
          <div className="space-y-6 py-4">
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <p className="text-sm text-blue-900 font-medium mb-2">Paso 1: Escanea el código QR</p>
              <p className="text-xs text-blue-700">
                Usa Google Authenticator, Microsoft Authenticator, Authy o cualquier app compatible con TOTP
              </p>
            </div>

            <div className="flex justify-center bg-white p-4 rounded-lg border-2 border-[#E0E0E0]">
              {qrCode && <Image src={qrCode} alt="QR Code 2FA" width={200} height={200} className="rounded" />}
            </div>

            <div className="space-y-2">
              <Label className="text-sm font-semibold text-[#002868]">O ingresa esta clave manualmente:</Label>
              <div className="flex gap-2">
                <Input value={secret} readOnly className="font-mono text-sm" />
                <Button type="button" variant="outline" size="icon" onClick={copySecret} className="flex-shrink-0">
        ...
        {step === 'verify' && (
          <form onSubmit={handleVerify} className="space-y-6 py-4">
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              <p className="text-sm text-green-900 font-medium mb-2">Paso 2: Verifica el código</p>
              <p className="text-xs text-green-700">Ingresa el código de 6 dígitos que aparece en tu aplicación</p>
            </div>
