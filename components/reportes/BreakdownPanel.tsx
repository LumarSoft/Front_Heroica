        ...
        >
            <div className="flex items-center justify-between mb-1.5">
                <div className="flex items-center gap-2 min-w-0">
                    ...
                    <span className={`text-sm font-medium text-slate-700 truncate ${isClickable ? "group-hover:text-slate-900" : ""}`}>
                        {item.name}
                        {isClickable && !isBack && (
...
function DrillDownBreadcrumb({
    ...
    return (
        <nav className="flex items-center gap-1.5 text-sm mb-1">
            <button onClick={onBack} className={`font-medium underline underline-offset-2 decoration-dotted transition-colors ${colorClass}`}>
                {root}
            </button>
