...
function bgFromIntensity(intensity: number): string {
  ...
  if (intensity < 0.9) return 'bg-indigo-500'
...
export function MonthlyCategoryBarChart({ data, moneda }: Props) {
  if (!data || data.length === 0) {
    return <p className="text-center italic text-slate-400 py-10 text-sm">No hay datos de categorías disponibles.</p>
  }
  ...
  const maxVal = Math.max(...categorias.flatMap(cat => meses.map(mes => matrix[cat][mes] ?? 0)))
      ...
      <table className="w-full text-xs border-collapse">
        ...
        <tbody>
          {categorias.map((cat, rowIdx) => {
            ...
            return (
              <tr key={cat} className={rowIdx % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}>
                {/* Nombre categoría sticky */}
                <td
                  className="sticky left-0 z-10 border-r border-gray-100 px-3 py-2 font-medium text-slate-700 whitespace-nowrap truncate max-w-[160px]"
                ...
                {meses.map(mes => {
                  ...
                  return (
                    ...
                    >
                      {val > 0 ? formatAbrev(val) : <span className="text-slate-300">·</span>}
                    </td>
                  )
                })}
        ...
        <tfoot>
          <tr className="border-t-2 border-gray-200 bg-slate-100">
            ...
            {meses.map(mes => {
              const mesTotal = categorias.reduce((acc, cat) => acc + (matrix[cat][mes] ?? 0), 0)
              return (
                <td key={mes} className="px-2 py-2 text-center font-bold text-slate-700 tabular-nums">
            ...
            <td className="border-l border-gray-200 px-3 py-2 text-right font-bold text-slate-700 tabular-nums">
              {formatAbrev(
                categorias.reduce((acc, cat) => acc + Object.values(matrix[cat]).reduce((a, b) => a + b, 0), 0),
              )}
            </td>
          </tr>
        </tfoot>
