...
export function SummaryCard({ label, value, accent, icon, sub, delta, invertDelta = false }: SummaryCardProps) {
    ...
    <div className="relative bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow duration-200 overflow-hidden print:break-inside-avoid">
      ...
      <div className="p-5">
        ...
        <div className="flex items-start justify-between mb-4">
          ...
          {icon && <span className={`p-2 rounded-xl ${a.iconBg} ${a.iconText} flex-shrink-0`}>{icon}</span>}
        </div>

        {/* Valor principal */}
        <p className={`text-2xl font-extrabold ${a.valueText} tabular-nums leading-none mb-3`}>{value}</p>
