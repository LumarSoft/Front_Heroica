...
function solvenciaInfo(
  ...
  if (deudas === 0)
    return {
      label: 'Sin deudas',
      desc: 'No hay deudas registradas actualmente',
      color: '#64748b',
      bg: 'bg-slate-50',
      text: 'text-slate-600',
  ...
  if (ratio > 2)
    return {
      label: 'Excelente',
      desc: 'Los créditos más que duplican las deudas',
      color: '#059669',
      bg: 'bg-emerald-50',
      text: 'text-emerald-700',
      border: 'border-emerald-200',
  ...
  if (ratio > 1)
    return {
      label: 'Positiva',
      desc: 'Los créditos cubren la totalidad de las deudas',
      color: '#0284c7',
      bg: 'bg-sky-50',
      text: 'text-sky-700',
  ...
  if (ratio > 0.5)
    return {
      label: 'Neutra',
      desc: 'Los créditos cubren parcialmente las deudas',
      color: '#d97706',
      bg: 'bg-amber-50',
      text: 'text-amber-700',
      border: 'border-amber-200',
  ...
  return {
    ...
    desc: 'Las deudas superan con creces los créditos',
    color: '#dc2626',
    bg: 'bg-rose-50',
    text: 'text-rose-700',
    border: 'border-rose-200',
...
function ConcentracionBars({
  ...
  return (
    <div className="space-y-2.5">
      ...
      {breakdown.length > 6 && (
        <p className="text-xs text-slate-400 italic text-right">+ {breakdown.length - 6} categorías más</p>
      )}
    </div>
  )
}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
            <div className={`px-5 py-3.5 border-b ${ingConc.border} ${ingConc.bg} flex items-center justify-between`}>
              <p className={`text-sm font-bold uppercase tracking-wider ${ingConc.text}`}>Concentración de Ingresos</p>
              <span
                className={`text-xs font-semibold px-2.5 py-1 rounded-full border ${ingConc.border} ${ingConc.bg} ${ingConc.text}`}
              >
                {ingConc.label}
              </span>
            </div>
            <div className="p-5">
              <p className="text-xs text-slate-400 mb-3">
                La categoría principal representa <strong className={ingConc.text}>{topIngPct.toFixed(1)}%</strong> del
                total de ingresos.
              </p>
          ...
          <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
            <div className={`px-5 py-3.5 border-b ${egrConc.border} ${egrConc.bg} flex items-center justify-between`}>
              <p className={`text-sm font-bold uppercase tracking-wider ${egrConc.text}`}>Concentración de Egresos</p>
              <span
                className={`text-xs font-semibold px-2.5 py-1 rounded-full border ${egrConc.border} ${egrConc.bg} ${egrConc.text}`}
              >
                {egrConc.label}
              </span>
            </div>
            <div className="p-5">
              <p className="text-xs text-slate-400 mb-3">
                La categoría principal representa <strong className={egrConc.text}>{topEgrPct.toFixed(1)}%</strong> del
                total de egresos.
              </p>
        ...
        <div className={`rounded-xl border ${solv.border} ${solv.bg} p-6`}>
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
            ...
            <div className="text-center sm:text-left min-w-[140px]">
              <p className="text-xs font-bold uppercase tracking-widest text-slate-500 mb-1">Ratio de Solvencia</p>
            ...
            <div className="flex-1 w-full space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-white/70 rounded-lg p-4 border border-white/60">
                  ...
                  <p className="text-lg font-extrabold text-indigo-600 tabular-nums">{formatMonto(creditos, moneda)}</p>
                </div>
                <div className="bg-white/70 rounded-lg p-4 border border-white/60">
                  <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Deudas a pagar</p>
                  <p className="text-lg font-extrabold text-orange-600 tabular-nums">{formatMonto(deudas, moneda)}</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="hidden print:block">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '1rem' }}>
          ...
          <div>
            ...
            >
              Concentración de Ingresos — <span style={{ color: ingConc.color }}>{ingConc.label}</span>
            </p>
          ...
          <div>
            ...
            >
              Concentración de Egresos — <span style={{ color: egrConc.color }}>{egrConc.label}</span>
            </p>
            <p style={{ fontSize: '8pt', color: '#64748b', marginBottom: '6px' }}>
