...
export function DeudasInterSucursalDialog({
  open,
  onOpenChange,
  sucursalId,
  ...
  const fetchDeudas = useCallback(async () => {
    setIsLoading(true);
    try {
      const url = API_ENDPOINTS.MOVIMIENTOS.GET_DEUDAS(
        sucursalId,
        fechaInicio,
        fechaFin,
      );
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[900px] max-h-[90vh] bg-white border-0 shadow-2xl rounded-2xl p-0 gap-0 overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-8 py-4">
          {isLoading ? (
            <div className="flex items-center justify-center h-32 text-[#8A8F9C] text-sm">
              Cargando deudas...
            </div>
          ) : !deudas?.length ? (
            <div className="flex flex-col items-center justify-center h-32 gap-2 text-[#8A8F9C]">
              ...
              <span className="text-sm">
                No hay deudas en el período seleccionado
              </span>
            </div>
          ) : (
            <table className="w-full text-sm">
              ...
              <tbody>
                {deudas.map((deuda) => (
                  ...
                  >
                    <td className="py-2.5 px-3 text-[#5A6070] whitespace-nowrap">
            ...
            {!isLoading && deudas?.length > 0 ? (
              <div className="flex flex-wrap items-center gap-6">
                <div className="flex items-center gap-2">
                  <span className="text-xs font-semibold text-[#5A6070] uppercase tracking-wider">
                    A cobrar:
                  </span>
                  <span className="text-sm font-bold text-emerald-600">
                    {formatMonto(totalAFavor)}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-xs font-semibold text-[#5A6070] uppercase tracking-wider">
