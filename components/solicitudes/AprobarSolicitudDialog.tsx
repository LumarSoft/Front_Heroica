    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[680px] bg-white border-0 shadow-2xl rounded-2xl p-0 overflow-hidden">
        ...
        <div className="px-8 py-6 space-y-5 max-h-[70vh] overflow-y-auto">
          ...
          {solicitud.detalles && (
            <div>
              <p className="text-[10px] font-bold uppercase tracking-wider text-[#9AA0AC] mb-2">
          ...
          {isPendiente && canAprobar && (
            <div>
              ...
              <Textarea
                ...
                placeholder="Obligatorio solo si rechazás la solicitud"
                className="min-h-[90px] resize-none rounded-lg border border-[#E0E0E0]"
              />
            </div>
          )}
              ...
              {(solicitud.historial ?? []).map(item => (
                <div key={item.id} className="rounded-lg border border-[#E0E0E0] bg-[#FAFBFC] px-3 py-2">
                  ...
                  <p className="text-xs text-[#5A6070]">
                    {item.usuario_nombre ?? 'Sistema'} · {format(new Date(item.created_at), 'dd/MM/yyyy HH:mm')}
                  </p>
            ...
            <div className="flex gap-3">
              {canEditCurrent && (
                <Button
                  ...
                  className="h-10 px-4 border-[#D8E3F8] text-[#002868]"
                >
