...
import {
  Clock, CheckCircle2, XCircle, X, Filter,
  UserPlus, UserMinus, Umbrella, FileText, AlertTriangle,
  GraduationCap, Package, TrendingUp, Star, Banknote, Loader2,
} from 'lucide-react'
...
const TIPO_CONFIG: Record<RhSolicitudTipo, TipoConfig> = {
  'Altas':               { label: 'Alta',               icon: <UserPlus className="w-4 h-4" />,       textColor: 'text-indigo-600',  bgColor: 'bg-indigo-50',  borderColor: 'border-l-indigo-400'  },
  'Bajas':               { label: 'Baja',               icon: <UserMinus className="w-4 h-4" />,      textColor: 'text-rose-600',    bgColor: 'bg-rose-50',    borderColor: 'border-l-rose-400'    },
  'Vacaciones':          { label: 'Vacaciones',          icon: <Umbrella className="w-4 h-4" />,       textColor: 'text-sky-600',     bgColor: 'bg-sky-50',     borderColor: 'border-l-sky-400'     },
  'Licencias':           { label: 'Licencia',           icon: <FileText className="w-4 h-4" />,       textColor: 'text-violet-600',  bgColor: 'bg-violet-50',  borderColor: 'border-l-violet-400'  },
  'Suspensiones':        { label: 'Suspensión',         icon: <X className="w-4 h-4" />,              textColor: 'text-orange-600',  bgColor: 'bg-orange-50',  borderColor: 'border-l-orange-400'  },
  'Apercibimientos':     { label: 'Apercibimiento',     icon: <AlertTriangle className="w-4 h-4" />,  textColor: 'text-red-600',     bgColor: 'bg-red-50',     borderColor: 'border-l-red-400'     },
  'Capacitaciones':      { label: 'Capacitación',       icon: <GraduationCap className="w-4 h-4" />,  textColor: 'text-emerald-600', bgColor: 'bg-emerald-50', borderColor: 'border-l-emerald-400' },
  'Pedido de uniforme':  { label: 'Uniforme',           icon: <Package className="w-4 h-4" />,        textColor: 'text-teal-600',    bgColor: 'bg-teal-50',    borderColor: 'border-l-teal-400'    },
  'Novedades de sueldo': { label: 'Novedad de sueldo',  icon: <TrendingUp className="w-4 h-4" />,     textColor: 'text-green-600',   bgColor: 'bg-green-50',   borderColor: 'border-l-green-400'   },
  'Incentivos y premios':{ label: 'Incentivo / Premio', icon: <Star className="w-4 h-4" />,           textColor: 'text-amber-600',   bgColor: 'bg-amber-50',   borderColor: 'border-l-amber-400'   },
  'Adelantos':           { label: 'Adelanto',           icon: <Banknote className="w-4 h-4" />,       textColor: 'text-cyan-600',    bgColor: 'bg-cyan-50',    borderColor: 'border-l-cyan-400'    },
}
...
function EstadoBadge({ estado }: { estado: RhSolicitudEstado }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold border flex-shrink-0 ${cls}`}>
      {icon}
      {estado}
    </span>
  )
...
function Chip({ children, highlight }: { children: ReactNode; highlight?: 'amber' | 'orange' | 'red' }) {
  ...
  return (
    <span className={`text-xs px-2 py-0.5 rounded-md border ${color}`}>{children}</span>
  )
}
...
function DetallesChips({ solicitud }: { solicitud: RhSolicitud }) {
  ...
  if (solicitud.tipo === 'Altas') {
    ...
    if (d.cantidad_dias) chips.push(<Chip key="dias">{Number(d.cantidad_dias)} día{Number(d.cantidad_dias) !== 1 ? 's' : ''}</Chip>)
    ...
    if (d.sueldo_actual !== undefined && d.sueldo_nuevo !== undefined)
      chips.push(<Chip key="s">{formatCurrency(d.sueldo_actual)} → {formatCurrency(d.sueldo_nuevo)}</Chip>)
    ...
    if (d.severidad) chips.push(<Chip key="s" highlight={sevHighlight}>Severidad: {sev}</Chip>)
    if (d.motivo)    chips.push(<Chip key="m">{String(d.motivo)}</Chip>)
  } else {
    ...
    Object.entries(d).forEach(([key, val]) => {
      ...
      const valueStr = typeof val === 'string' && /^\d{4}-\d{2}-\d{2}/.test(val)
        ? formatFecha(val)
        : String(val)
      chips.push(<Chip key={key}>{label}: {valueStr}</Chip>)
    })
  }
...
function SolicitudCard({ solicitud }: { solicitud: RhSolicitud }) {
  ...
  return (
    <div className={`bg-white rounded-xl border border-[#E5E9F0] border-l-4 ${config.borderColor} p-4 hover:shadow-sm transition-shadow`}>
      {/* Encabezado */}
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-2">
          <span className={`flex-shrink-0 p-1.5 rounded-lg ${config.bgColor} ${config.textColor}`}>
            {config.icon}
          </span>
      ...
      <div className="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-xs text-[#888]">
        ...
        <span>Solicitada por <strong className="text-[#5A6070]">{solicitud.usuario_nombre}</strong></span>
        {solicitud.resuelto_por_nombre && solicitud.estado !== 'Pendiente' && (
          <span>
            {solicitud.estado === 'Aprobada' ? 'Aprobada' : solicitud.estado === 'Rechazada' ? 'Rechazada' : 'Cancelada'}
            {' '}por{' '}
            <strong className="text-[#5A6070]">{solicitud.resuelto_por_nombre}</strong>
...
export function HistorialTab({ personalId }: HistorialTabProps) {
  ...
  const selectCls = 'h-8 rounded-lg border border-[#D8E3F8] bg-white px-2.5 text-xs text-[#444] focus:outline-none focus:ring-2 focus:ring-[#002868]/20 cursor-pointer'

  return (
    <div>
      {/* Encabezado y filtros */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
        <p className="text-[11px] font-semibold uppercase tracking-wider text-[#9AA0AC]">
